...
>
> > **해결방안**<br>: 동일하게 구현하는 데는 현재 역량으로 한계가 있었다. 대신, 효과를 단순화하여 표현하는 것으로 해결.

> **문제 ②** <br>헤더에서 어바웃 첫 페이지까지 자연스럽게 넘어가는 효과를 주었는데, 그라디언트 값으로 해결하기에는 부자연스러움.
>
> > **해결방안**<br>: 두 섹션 사이에 새로운 배경 영역을 div로 추가. <br> GSAP를 이용하여 스크롤 배경 효과를 적용.

**2. [contact us] page**

> **문제 ①** <br>값의 유무에 따라 알림창을 다르게 뜰 수 있도록 if 문을 이용하여 자바스크립트 코드를 작업하였으나, false 값만 출력됨.
>
> > **해결방안** <br>:처음에는 input만 선언하여 작업하였으나, 모든 input의 아이디를 선언하여, 세 개의 입력값 중 하나라도 빈 문자열이 있는 경우로 확인하도록 작성하여 해결.

> **문제 ②** <br>placeholder 상태에서 input 값을 입력했을 때 글자 색상이 변해야 하는데, 설정하지 않은 placeholder의 색상까지 값이 적용.
>
> > **해결방안** <br>:input 기본값에 컬러를 설정하여 placeholder의 기본값을 유지.
